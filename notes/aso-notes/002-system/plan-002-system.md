...
- **Module layout**: `server/lib/sync/{paths,hash,upsert-decision,files,dispatcher,sweeper,ingest,folder-sync,workspace}.ts`. Pure seams (`paths`, `hash`, `decideUpsert`, dispatcher selection, sweeper query builder) are unit-tested; DB handlers are e2e-tested by driving `handleFileUpsert`/`handleFileUnlink`/`startupScan` directly against a temp notes dir, plus one real chokidar smoke spec (`test/e2e/folder-sync.spec.ts`).
...
## Post-M9 hardening & UX (2026-07-26 → 2026-07-27)

Work done after the M9 live verification, as the app moved into daily dogfooding.

### Setup & DX

- **README setup rewritten**: prerequisites (Node 24, pnpm 11, Docker), 5-step guide, full env-var table (required vs optional with defaults), self-hosting note. `apps/web/.env.example` completed from an email-only stub to every config point with dev-working defaults.
- **Redis added to `docker-compose.yml`** (`redis:7-alpine`, port 6379) — without it the sweeper is disabled and synced notes sit `pending` forever; a silent onboarding trap for fresh clones.
- **Turnstile skipped in dev** (`cdf7809`): `POST /api/auth/captcha` early-returns success when `NODE_ENV=development`; signup form hides the widget and skips the token check. UI signup works locally.
- **portless for named HTTPS dev URL**: `pnpm dev` now serves `https://asonotes.localhost` (trusted local CA) via portless (`"dev": "portless"` in `apps/web/package.json`, raw fallback `pnpm --filter web dev:app` → `localhost:3000`). Root `portless.json` + per-package `"portless"` key (the package.json key is the one that takes effect).
- **better-auth `baseURL`**: set from `env.public?.siteUrl ?? NUXT_PUBLIC_SITE_URL` (site origin, not `/api/auth` — better-auth appends `basePath` itself). The defensive `?.` matters: the test harness passes a partial runtime config, and the unguarded access crashed the whole e2e suite.

### Branding & app chrome (commit `6ea404c`)

- **`app-header.vue`** in the default layout: graph-outline icon + `aso-notes` wordmark, Chat/Notes/Graph nav with active-route underline, session-gated. Replaces the boilerplate "My app" logo.
- **Site identity**: `site.name`/`description` in `nuxt.config.ts` fixes the `%siteName` title template; `appInfo` updated; favicon untouched.
- **Manual ingestion trigger**: `POST /api/notes/process` dispatches pending notes in a folder immediately (BullMQ when Redis present, inline otherwise; test seam `setProcessTestDispatcher`). Notes page shows "⚡ Process N pending" for the selected folder.

### Chat UX overhaul (commit `5b65297`)

- **Layout**: redundant per-page headers removed from Chat/Notes/Graph (global nav carries section context); page heights use `calc(100dvh - 3.5rem)` against the fixed `h-14` app-header — the old `h-screen` overflowed by the header height. Activity panel capped (`max-h-72`, internal scroll) so it can't squash the thread.
- **Observability**: activity rows are compact one-liners (status dot, tool name, args summary, result count like `12 notes`); per-row debug expand shows **raw input + output JSON**; expand-all/collapse-all; panel collapsible. Past conversations reconstruct the activity log from persisted assistant `tool_calls` + `tool` messages — tool-call turns are no longer rendered as empty chat bubbles (the "4 mystery bubbles" bug).
- **Edit previous message**: pencil action on persisted user messages → prefilled input with notice → `POST /api/conversations` with `editFromMessageId` deletes that turn and everything after it (`truncateFromMessage` in `run-agent.ts`) before the new turn runs. e2e-tested including foreign-message rejection.
- **Archive conversations**: `archived_at` migration (`1785089252948`), `PATCH /api/conversations/[id] { archived }`, list excludes archived unless `?archived=true`, sidebar hover-archive + collapsible Archived section with restore.
- **Stream lifecycle**: `useChat` holds an `AbortController`; page unmount cancels the stream. Fixes the unhandled component-update error when navigating away mid-stream (the stream's completion used to fire `useFetch.refresh` on an unmounted page).
- **Citations across canonical reload**: after a stream completes, the conversation detail is refreshed (real message ids for edit, persisted tool activity) and streamed citations are preserved by content match in `loadMessages`.

### Notes page additions

- **Add note**: inline name input at the top of the NOTES panel → slugified `.md` in the selected folder (root when unselected) → opens in edit mode. `PUT /api/notes/:path` now **creates** files (the `loadNoteByPath` 404 guard moved into the GET branch); collision shows an inline error. Traversal guard semantics clarified: raw `..` segments never reach the handler (URL normalization), encoded `%2E%2E` is rejected by router (404) or the path guard (400) — test asserts rejection + nothing written.

### Test-suite hygiene

- OpenRouter factory tests made hermetic (`vi.stubEnv` + `unstubAllEnvs`) so a developer's `.env.local` (e.g. `NUXT_OPENROUTER_CHAT_MODEL`) can't break them.
- Known flake remains: `test/e2e/notes-watcher.spec.ts` chokidar timing test (passes in isolation).
- Suite after post-M9 work: **302 passed** across 45 files; lint clean.

### Deferred / open (updated)

From the original list, still open: doom-loop detection, concept merge UI, top-K concept injection, `youtube-transcript` pipeline (+ TikTok, Excalidraw sources), token streaming for chat answers, pg_trgm fallback for concept search.

Newly observed:

- **Sync workspace resolution is boot-time**: first-ever signup requires a dev-server restart before sync engages (workspace resolved once at boot). Lazy-retry candidate.
- **Markdown rendering is unsanitized** (`marked` straight to `v-html`) — acceptable for single-user, revisit before multi-tenant.
- **Landing page i18n keys are untranslated boilerplate** (`landingpage.hero.title` renders literally) — needs a real copy pass or removal of the landing page.
- **Post-signup lands on `/`** instead of `/chat` (the signed-in redirect only covers visits, not the post-signup landing).
- **Relations have no `weight` column** (M8 returns `weight: 1` placeholder) — schema migration candidate.
- **Notes are not versioned**: editing a note overwrites the file; no history. Git-backed notes dir would give history for free (the notes dir is intentionally outside the repo's git).

## M10 — Provider registry + per-use model configs (LLM/Embedding)

**Status: IN PROGRESS (2026-07-27)**

### Goal

Decouple the three LLM/embedding call sites (agent conversation, ingestion extraction, embeddings) from OpenRouter and from each other: per-use provider + model config, with Ollama as the first second backend (local models, e.g. gemma via `http://localhost:11434`).
...
- **Registry + strategy**: `server/lib/ai/registry.ts` exposes `createLLMProvider(role: 'agent' | 'extraction', env)` and `createEmbeddingProvider(env)`; selects backend by env, resolves fallback chains. Existing `LLMProvider`/`EmbeddingProvider` interfaces in `types.ts` are unchanged — the surface area was verified to cover Ollama native (`/api/chat` messages+tools, `format` json schema, `/api/embed` batching).
- **Ollama native clients**: `ollama-llm.ts` (`/api/chat`; maps `tool_calls[].function.arguments` object → OpenAI-style JSON-string `ToolCall`, client-generated tool-call ids, `toolChoice:'none'` ≈ omit tools, `responseFormat.json_schema` → `format: <schema>`), `ollama-embedding.ts` (`/api/embed`).
- **Rewiring**: `pipeline/singleton.ts` → `createLLMProvider('extraction')`; `agent/providers.ts` → `createLLMProvider('agent')`. OpenRouter providers untouched; defaults preserve current behavior exactly.
- **Embeddings stay remote for now** (halfvec(2048) columns); the registry makes a future local embedding model an env-only change (dimension guardrail deferred).
- **Tests**: unit for Ollama clients (fetchFn injection) + registry selection/fallbacks; no live Ollama in the suite.
- **Live verification gate**: point agent+extraction at a local Ollama model, ingest one note, ask one question — must confirm tool calling AND `format` json schema work on the user's Ollama build (compat risk noted from ollama/ollama#10001).
