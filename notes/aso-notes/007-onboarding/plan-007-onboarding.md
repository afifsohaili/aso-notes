...
- [x] **Phase 9 — Auth UI + session** — DONE 2026-08-01. Redesigned login/signup with shared `auth-card`, `auth-toast` replacing `floating-dialog`, password visibility toggles, full i18n. Signup navigates straight into the app when a session exists (verify-email state otherwise). Better-auth session: 1-year expiry, rolling daily `updateAge`.
- [x] **Phase 10 — Nav polish, vocab default, rebuild queue purge** — DONE 2026-08-01. Header nav links all carry heroicons (chat, document, queue, share, cog) with a visible `hover:bg-gray-100` pill. Default vocabulary strategy is now `full` (was `top-k`). Rebuild endpoint purges ingestion jobs per note id after the wipe commits (`purgeIngestionJobs` in `server/lib/sync/dispatcher.ts`), returning `jobsPurged`; danger-zone UI surfaces the count. **BullMQ gotcha recorded**: `Queue#remove` silently no-ops on locked jobs while resolving — always `Job#remove` (throws when locked; caught + skipped). Verified live against the dev Redis: 10 stuck failed jobs present, one purged end-to-end. Removed `locales/my.json` and the unused `language-switcher.vue`; i18n is en-only.
...
- **Bug found by browser verification (fixed)**: BullMQ `add()` dedupes by `jobId = noteId`, so a lingering FAILED job (kept 7 days via `removeOnFail`) made every re-dispatch a silent no-op — notes stuck at `queued` after a failed ingestion + reset. The BullMQ dispatcher now removes terminal-state (`failed`/`completed`) jobs before re-adding. Covered by 2 new tests in `test/e2e/notes-dispatcher.spec.ts`.
- **Bug found by browser verification (fixed)**: the smoke test's 3-minute timeout could never beat the sweeper's 5-minute `PENDING_SETTLE_INTERVAL` — the verify step always timed out against the real daemon (specs drove `ingestNote` directly, masking it). The smoke-test GET poll now backdates the pending note's `updated_at` so the normal sweep path dispatches it within the timeout; the note still travels watcher → sweeper → queue → worker → pipeline. Covered by a new settle-eligibility test in `test/e2e/onboarding-smoke-test.spec.ts`. Full suite after both fixes: **668 tests passed**.
- **Bug found by browser verification (fixed)**: chokidar can deliver the upsert for a recreated file BEFORE the grace-delayed unlink handler (delete+recreate within `UNLINK_GRACE_MS` — exactly the smoke-test rewrite pattern); the blind unlink then wiped the fresh row and the note stuck until the next startup scan. `handleFileUnlink` now stats the path first and no-ops if the file exists again. Two stale test simulations updated to remove the file before driving unlink. Covered by a new delete+recreate race test in `test/e2e/notes-sync.spec.ts`. Full suite: **86 files / 669 tests passed**.
